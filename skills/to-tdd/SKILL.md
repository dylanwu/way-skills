...
- **Name the model on every dispatch — implementers and reviewers alike.** An omitted model silently inherits the session's, usually the most capable and most expensive; that is how a whole quota goes at once. Transcription-grade slices (the brief carries the exact test and near-exact implementation, 1–2 files) → cheapest tier. Prose-driven implementation or multi-file integration → standard tier; cheap models take 2–3× the turns on multi-step prose work and cost more overall — turn count beats token price. Design judgment (the seam itself is in question) → not a dispatch at all; that is driver work, settled with the user.
- **Review between cycles — dispatch it, don't read it, and price it separately.** Reviewing inline burns the context you need for driving; dispatched, the diff and the evaluation stay in the reviewer's context and only findings come back. Send it to a *different* agent than the one that wrote the slice, briefed with the slice's intent, the diff range and the test evidence — never this session's history. The two review questions price differently: *did it do what was asked and nothing more* is mechanical comparison against the brief, which the cheapest tier handles; *is this code you would keep* is judgement, and wants roughly the tier that would have designed the seam. A transcription-grade slice hardly needs the second — its design was settled in the brief before dispatch. Never fix findings in the driver session; send them back.
